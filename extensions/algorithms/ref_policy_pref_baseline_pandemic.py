...
from extensions.algorithms.ref_pref_pandemic_train_policy import ex
from extensions.environments.pandemic_configs import get_pandemic_env_gt_rew
import extensions.algorithms.ref_pref_pandemic_train_policy
import extensions.algorithms.ref_pref_pandemic_unique_id_state as unique_id_state
import time
...
iterative_ex = Experiment("ref_policy_pref_baseline", save_git_info=False)
...
def main(
    ...
    for i in range(10):
        ...
        if i == 0:
            reference_result = ex.run(
                config_updates={
                    ...
                    "checkpoint_to_load_current_policy": "data/logs/pandemic/2025-05-12_17-32-56/checkpoint_000025",
                    "seed": seed,
            ...
            reference_result = ex.run(
                config_updates={
                    ...
                    "om_divergence_coeffs": om_divergence_coeffs_1,
                    "checkpoint_to_load_policies": checkpoint_to_load_policies,
                    "checkpoint_to_load_current_policy": "data/logs/pandemic/2025-05-13_00-30-10/checkpoint_000025",
                    "seed": seed,
                    "experiment_tag": experiment_tag,
                    "om_divergence_type": om_divergence_type,
                    "num_rollout_workers": num_rollout_workers,
        ...
        reference_only_result = ex.run(
            config_updates={
                "env_to_run": env_to_run,
                "level":level,
                "reward_fun": reward_fun,
                "exp_algo": exp_algo,
                "om_divergence_coeffs": [1.0],
                "checkpoint_to_load_policies": checkpoint_to_load_policies,
                "checkpoint_to_load_current_policy": checkpoint_to_load_current_policy,
                "seed": seed,
                "experiment_tag": experiment_tag,
        ...
        eval_batch_reference_only = reference_only_result.result[2]
        
        #TODO: our reward model might need to input a history of obs (possibly just last obs) instead of just the current obs
        print (len(eval_batch_reference_only["current"]))
        print (len(eval_batch_reference["current"]))
        if i == 0:
            reward_model.add2replay(eval_batch_reference_only["current"][:len(eval_batch_reference["current"])], eval_batch_reference["current"])
        else:
            reward_model.update_params(eval_batch_reference_only["current"][:len(eval_batch_reference["current"])],eval_batch_reference["current"], iteration=i,use_minibatch=True)
        ...
        print ("======================")
        print ("saving reference only checkpoint to:", reference_only_result.result[1])
