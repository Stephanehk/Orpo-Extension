...
from extensions.algorithms.sas_preloaded_reward_train_policy_pandemic import ex
from extensions.environments.pandemic_configs import get_pandemic_env_gt_rew
import extensions.algorithms.sas_preloaded_reward_train_policy_pandemic
import extensions.algorithms.sas_preloaded_reward_unique_id_state_pandemic as unique_id_state
import time
...
iterative_ex = Experiment("iterative_reward_design_pandemic_preloaded", save_git_info=False)
...
def main(
    ...
    #unique_id=28 corrosponds to the experiment: IRD with pandemic simulator, unique_id_state=28, 1521 prefs per iter + bigger RM (10343226)
    reward_model.load_state_dict(torch.load(f"active_models/reward_model_{575}.pth"))
    reward_model.save_params()
