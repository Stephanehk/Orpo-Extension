...
from extensions.algorithms_tomato_pref_against_uniform.train_policy import ex
import extensions.algorithms_tomato_pref_against_uniform.train_policy
import extensions.algorithms_tomato_pref_against_uniform.unique_id_state as unique_id_state
import time

import warnings
warnings.filterwarnings("ignore")

iterative_ex = Experiment("iterative_reward_design_tomato_pref_against_unif", save_git_info=False)
...
def main(
    ...
    wrapped_policy = RandomActionPolicy(policy, random_prob=1.0)
    ...
    for i in range(10):
        ...
        unif_policy_result = ex.run(
            config_updates={
                ...
                "checkpoint_to_load_policies": None,
                "checkpoint_to_load_current_policy": random_checkpoint_path,
                "seed": seed,
        ...
        eval_batch_over_opt = unif_policy_result.result[2]
        ...
        unif_policy_rollouts = eval_batch_over_opt["current"][:len(eval_batch_reference["current"])]
        reference_rollouts = eval_batch_reference["current"]
       
        reward_model.update_params(unif_policy_rollouts,reference_rollouts, iteration=i)
        ...
        print ("======================")
        print ("saving over opt checkpoint to:", unif_policy_result.result[1])
