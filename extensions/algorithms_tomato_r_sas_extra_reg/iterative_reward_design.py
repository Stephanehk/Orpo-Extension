...
from extensions.algorithms_tomato_r_sas_extra_reg.train_policy import ex
import extensions.algorithms_tomato_r_sas_extra_reg.train_policy
import extensions.algorithms_tomato_r_sas_extra_reg.unique_id_state as unique_id_state
import time

import warnings
warnings.filterwarnings("ignore")

iterative_ex = Experiment("iterative_reward_design_tomato_sas_extra_reg", save_git_info=False)
...
def main(
    ...
    for i in range(10):
        ...
        over_opt_rollouts = eval_batch_over_opt["current"][:len(eval_batch_reference["current"])]
        reference_rollouts = eval_batch_reference["current"]
        if i == 0:
            original_reference_rollouts = reference_rollouts
        winrate_against_reference = reward_model.check_stopping_condition(over_opt_rollouts, reference_rollouts)
        winrate_against_original_reference = reward_model.check_stopping_condition(over_opt_rollouts, original_reference_rollouts)
        print ("winrate against reference:", winrate_against_reference)
        print ("winrate against original reference:", winrate_against_original_reference)
        print ("======================")
        #save to file
        unique_id = unique_id_state.state["unique_id"]
        with open(f"winrate_against_references_{unique_id}.txt", "a") as f:
            f.write(f"Againt Reference, Iteration {i}: {winrate_against_reference}\n")
            f.write(f"Againt Original Reference, Iteration {i}: {winrate_against_original_reference}\n\n")

        reward_model.update_params(over_opt_rollouts,reference_rollouts, iteration=i)
