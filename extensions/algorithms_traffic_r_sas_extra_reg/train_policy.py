...
def create_env_pandemic(config):
    ...
    return RewardWrapper(base_env, reward_model=config.get("reward_model", "custom_pandemic_sas"), unique_id=unique_id_state.state["unique_id"])
...
def create_env_traffic(config,flow_params, reward_specification, reward_fun, reward_scale):
    ...
    # base_env = create_simple_example(config)
    base_env = create_env()
    ...
    # print (base_env)
    # assert False
    return RewardWrapper(base_env, reward_model=config.get("reward_model", "custom_traffic_sas"), unique_id=unique_id_state.state["unique_id"])

ex = Experiment("orpo_traffic_experiments_sas_extra_reg", save_git_info=False)
...
def main(
    ...
    eval_batch = synchronous_parallel_sample(
        worker_set=algorithm.workers, max_env_steps=193*2*80
    )
