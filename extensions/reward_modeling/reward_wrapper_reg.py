...
class RewardModel(nn.Module):
    ...
    def update_params_gt(self, train_batch1, train_batch2, iteration, debug_mode=False,use_minibatch=False, force_n_epochs=None, psuh2zero = 10):
            ...
            if use_minibatch:
                ...
                for i in range(0, len(buffer_items), BATCH_SIZE):
                    ...
                    for item in batch:
                        ...
                        batch_loss += torch.nn.functional.mse_loss(traj_1_preds, traj1["reward_for_pref"].flatten().to(self.device)) + torch.nn.functional.mse_loss(traj_2_preds, traj2["reward_for_pref"].flatten().to(self.device))
                ...
                all_losses.append(reward_model_loss/len(self.replay_buffer.buffer))
