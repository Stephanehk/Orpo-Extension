...
class RewardModel(nn.Module):
    ...
    def add2replay(self, train_batch1, train_batch2, train_batch3):
        assert len(train_batch1) == len(train_batch2) == len(train_batch3)#doesn't necesserily have to be the case, but cleans up implementation
        ...
        #reference policy rollouts
        rewards_sequences3,acs_sequences3,obs_sequences3,new_obs_sequences3, reward_sequences_for_prefs3, proxy_reward_seq3 = self.get_batch_sequences(train_batch3,batch_seq_lens)

        trajectory_pairs = [(i, j) for i in range(num_sequences-1) for j in range(num_sequences-1)]
        
        if self.env_name != "tomato":
            raise ValueError("This function is not implemented for the pandemic/other environments")

        #select 181 pairs between unconstrained and constrained PPO rollouts
        selected_is = np.random.choice(list(range(len(trajectory_pairs))), size=181, replace=False)
        ...
        #select 180 pairs between constrained PPO rollouts and reference policy rollouts
        selected_is = np.random.choice(list(range(len(trajectory_pairs))), size=180, replace=False)
        trajectory_pairs = [trajectory_pairs[i] for i in selected_is]
        for indices_pair in trajectory_pairs:
            traj1 = self._create_sample_batch(
                rewards_sequences2,
    ...
    def update_params(self, train_batch1, train_batch2,train_batch3, iteration, debug_mode=False):
        ...
        if not debug_mode:
            self.add2replay(train_batch1, train_batch2,train_batch3)
...
class RewardWrapper(Wrapper):
    ...
    def step(self, action):
        ...
        if "custom" in self.reward_model:
            ...
            if "pandemic" in self.reward_model:
